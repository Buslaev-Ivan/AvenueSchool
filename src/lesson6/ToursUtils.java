...
public class ToursUtils {
    ...
    //вывод фильтр продолжтетельность дни
    ...
    //вывод туров по кол-ву звезд и типу транспорта
    ...
    public static void printToursByBudget(String[][] tours, int budget) {
        for (int i = 0; i < tours.length; i++) {
            // "1200000 руб "  в пятой колонке
            //int budget
            if (tours[i][0] != null) {

                int spaceIndex = tours[i][5].indexOf(" ");// узнали позицию пробела
                String priceStr = tours[i][5].substring(0, spaceIndex);//осталась только цена без " руб"
                int priceCurrentTour = Integer.parseInt(priceStr);//преобразовали текстувую цену в число
